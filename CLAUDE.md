...
| `src/jogo/` | Páginas do jogo: `Fichas.tsx` (o painel "Suas fichas" do perfil e a página da mesa `/jogo/mesa`, do mestre, com as fichas dos jogadores e os convites; o estado da tela sai de `fichasState.ts`, puro e testado), `access.ts` (o `/users/me` da conta, uma pergunta por conta: o que mostrar vem em `features`, decidido pela API); `apiClient.ts` fala com a API do jogo (testado no node, com as respostas de `contract/ficha-sync.json`, cópia do contrato do backend: mudou lá, muda aqui no mesmo par de PRs). |
