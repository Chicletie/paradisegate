...
| `src/jogo/` | Páginas do jogo: `Fichas.tsx` (Minhas Fichas em `/jogo/fichas`, com os convites pra admin; o estado da tela sai de `fichasState.ts`, puro e testado); `apiClient.ts` fala com a API do jogo (testado no node, com as respostas de `contract/ficha-sync.json`, cópia do contrato do backend: mudou lá, muda aqui no mesmo par de PRs). |
