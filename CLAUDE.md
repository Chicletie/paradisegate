...
12. **Este repositório é só o front; a regra de negócio do jogo mora no backend** (a API do jogo,
    repo `pg-backend`). Quem vê o quê, liga/desliga de funções, nome tirado da ficha, padrões,
    cortes, ordem das listas, nome pronto pra mostrar ("@username"), contagens e mensagens de erro
    de regra: a API decide e manda pronto (ex.: `features` do `/users/me`, `owner.name`, o nome da
    ficha, `POST /character-sheets/import`). O site desenha o que vem e cuida só do que é do
    navegador: a gaveta da ficha no aparelho, a sincronização, ler o arquivo escolhido, a tela.
    Antes de escrever uma regra aqui, pergunte se a API pode decidir; se pode, é lá.
...
| `src/jogo/` | Páginas do jogo: `Fichas.tsx` (o painel "Suas fichas" do perfil e a página da mesa `/jogo/mesa`, do mestre, com todas as fichas da mesa; convite não é aqui, é no editor do mestre, repo `arvore`, que pergunta à API do jogo; o estado da tela sai de `fichasState.ts`, puro e testado), `access.ts` (o `/users/me` da conta, uma pergunta por conta: o que mostrar vem em `features`, decidido pela API); `apiClient.ts` fala com a API do jogo (testado no node, com as respostas de `contract/ficha-sync.json`, cópia do contrato do backend: mudou lá, muda aqui no mesmo par de PRs). |
