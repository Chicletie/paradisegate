...
As fichas moram no perfil, como a Página de Perfil previa: o painel **Suas fichas** (`/wiki/_perfil#fichas`, com atalho no topo e "Minhas fichas" no menu da conta; `/jogo/fichas` leva pra lá). Pro mestre (admin da API do jogo) aparece sempre; pro jogador, só com `VITE_FEATURE_FICHAS=1`; quem não está na mesa não vê nada. A página da mesa, **Fichas da mesa** (`/jogo/mesa`), é do mestre: link na barra de navegação só pra ele, antes de Linha do tempo; mesma casca do perfil. Estilos em `src/styles/jogo.css`.
...
- **Fichas dos jogadores** (página da mesa): as fichas dos outros jogadores, a mais recente em cima, na mesma linha dos Favoritos com "@jogador · atualizada em…" (o dono em `ink` 500). Abrem na ficha em só leitura: a barra da ficha diz "Ficha de @jogador: só leitura, o que mudar aqui não vai pra conta" em dourado; se o jogador muda enquanto está aberta, "mudou desde que você abriu. Atualizar".
...
- **Jogadores da mesa** (página da mesa): nota em itálico `muted`, campo "E-mail do jogador" com "Convidar", e a lista (e-mail, ponto dourado "já entrou" ou vazio "ainda não entrou", quem convidou, "Remover").
